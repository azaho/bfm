...
spec1=${spec_options[$(((idx1 / (n_rs * n_sl * n_lr)) % n_fb))]}
use_cls_token1=${use_cls_token_options[$(((idx1 / (n_rs * n_sl * n_lr * n_fb * n_sp)) % n_uct))]}
p_unmasked1=${p_unmasked_options[$(((idx1 / (n_rs * n_sl * n_lr * n_fb * n_sp * n_uct)) % n_pum))]}
...
spec2=${spec_options[$(((idx2 / (n_rs * n_sl * n_lr)) % n_fb))]}
use_cls_token2=${use_cls_token_options[$(((idx2 / (n_rs * n_sl * n_lr * n_fb * n_sp)) % n_uct))]}
p_unmasked2=${p_unmasked_options[$(((idx2 / (n_rs * n_sl * n_lr * n_fb * n_sp * n_uct)) % n_pum))]}

echo "Job 1 - RS: $random_string1, SL: $symmetric_loss1, LR: $learning_rate1, FB: $future_bin_idx1, SPEC: $spec1, UCT: $use_cls_token1, PUM: $p_unmasked1"
echo "Job 2 - RS: $random_string2, SL: $symmetric_loss2, LR: $learning_rate2, FB: $future_bin_idx2, SPEC: $spec2, UCT: $use_cls_token2, PUM: $p_unmasked2"
