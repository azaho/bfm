...
class ElectrodeDataEmbeddingFFT(ElectrodeDataEmbedding):
    ...
    def add_subject(self, subject, sampling_rate):
        ...
        self.normalization_means[subject_identifier] = nn.Parameter(torch.zeros(subject.get_n_electrodes(), self.max_frequency_bin, dtype=self.dtype, device=self.device), requires_grad=self.normalization_requires_grad)
        self.normalization_stds[subject_identifier] = nn.Parameter(torch.ones(subject.get_n_electrodes(), self.max_frequency_bin, dtype=self.dtype, device=self.device), requires_grad=self.normalization_requires_grad)
    ...
    def _forward_fft(self, x, sampling_rate):
        ...
        assert n_samples % timebin_samples == 0, f"n_samples must be a multiple of timebin_samples, got {n_samples} and {timebin_samples}"
    ...
    def calculate_electrode_normalization(self, electrode_data, sampling_rate):
        ...
        electrode_data = electrode_data.to(self.device)
    
        electrode_data = self._forward_fft(electrode_data, sampling_rate)
        batch_size, n_electrodes, n_timebins, n_frequency_bins = electrode_data.shape

        electrode_data = electrode_data.permute(1, 0, 2, 3).reshape(n_electrodes, -1, n_frequency_bins) # flatten the batch dimension
        return electrode_data.mean(dim=1), electrode_data.std(dim=1)
    ...
    def forward(self, subject_identifier, electrode_indices, electrode_data, max_n_electrodes=None, normalization_means=None, normalization_stds=None):
        ...
        if normalization_means is None:
            normalization_means = self.normalization_means[subject_identifier][electrode_indices]
        if normalization_stds is None:
            normalization_stds = self.normalization_stds[subject_identifier][electrode_indices]
        ...
        electrode_data = self._forward_fft(electrode_data, self.sampling_rates[subject_identifier]) # shape: (batch_size, n_electrodes, n_timebins, max_frequency_bin)
        batch_size, n_electrodes, n_timebins, n_frequency_bins = electrode_data.shape

        electrode_data = electrode_data - normalization_means.view(1, n_electrodes, 1, n_frequency_bins)
        electrode_data = electrode_data / (normalization_stds.view(1, n_electrodes, 1, n_frequency_bins) + self.std_smoothing)
