...
from model_model import GranularModel, BinTransformer, CrossModel, FFTaker
...
assert n_downsample_factor == 1, "n_downsample_factor must be 1, not supported in the FFTaker class yet."
...
if not model_config['electrode_embedding']['spectrogram']:
    ...
    assert n_downsample_factor == 1, "n_downsample_factor must be 1, not supported in the FFTaker class yet."
    bin_transformer = FFTaker(
        d_model=192,#model_config['transformer']['d_model'],
        overall_sampling_rate=2048,
        sample_timebin_size=model_config['sample_timebin_size'],
        max_frequency_bin=model_config['max_frequency_bin'],
        power=model_config['electrode_embedding']['spectrogram_power']
    ).to(device, dtype=model_config['dtype'])
...
model = GranularModel(
    ...
    identity_init=model_config['init_identity'],
    n_cls_tokens=2 # XXX make this be config cls_token in the model_config
).to(device, dtype=model_config['dtype'])
...
def calculate_loss_function(batch, output_accuracy=True):
    ...
    random_electrodes = torch.randperm(n_electrodes) # n_electrodes is already clipped to max_n_electrodes by the dataloader
    ...
    output_a_electrode_output, output_a_cls_token = model(bin_transformed_data_a[:, :, :-1, :], embeddings_a, return_cls_token=True) # shape: (batch_size, n_electrodes, n_timebins-1, sample_timebin_size*SR//n_downsample_factor)
    
    #output_a = bin_transformed_data_a[:, :, :-1, :]
    if training_config['normalize_features']:
        output_a_electrode_output = output_a_electrode_output / (torch.norm(output_a_electrode_output, dim=-1, keepdim=True) + 0.001)
        output_a_cls_token = output_a_cls_token / (torch.norm(output_a_cls_token, dim=-1, keepdim=True) + 0.001)


    output_b_electrode_output, output_b_cls_token = model(bin_transformed_data_b[:, :, 1:, :], embeddings_b, return_cls_token=True) # shape: (batch_size, n_electrodes, n_timebins-1, sample_timebin_size*SR//n_downsample_factor)

    output_a_cls_token = output_a_cls_token[:, 0:1] # Select the first CLS token
    output_b_cls_token = output_b_cls_token[:, 1:2] # Select the second CLS token

    if training_config['normalize_features']:
        output_b_electrode_output = output_b_electrode_output / (torch.norm(output_b_electrode_output, dim=-1, keepdim=True) + 0.001)
        output_b_cls_token = output_b_cls_token / (torch.norm(output_b_cls_token, dim=-1, keepdim=True) + 0.001)

    similarity_b = output_b_cls_token.permute(1, 2, 0, 3) @ output_a_cls_token.permute(1, 2, 3, 0) # shape: (n_electrodes, n_timebins-1, batch_size, batch_size)
    ...
    expanded_arange = torch.arange(batch_size).unsqueeze(0).repeat(1, n_timebins-1, 1).to(model.device, dtype=torch.long).reshape(-1)
    loss_b = torch.nn.functional.cross_entropy(similarity_b.view(-1, batch_size), expanded_arange)
    losses['contrastive_cls'] = loss_b
    if output_accuracy:
        accuracy_b = (similarity_b.view(-1, batch_size).argmax(dim=-1) == expanded_arange).float().mean()
        losses['accuracy_cls'] = accuracy_b


    output_a = torch.cat([output_a_electrode_output, output_a_cls_token], dim=1) # shape: (batch_size, n_electrodes, n_timebins-1, sample_timebin_size*SR//n_downsample_factor)
    output_b = cross_model(embeddings_b, output_a, positions_offset=0) # shape: (batch_size, n_electrodes, n_timebins-1, sample_timebin_size*SR//n_downsample_factor)
...
for epoch_i in range(training_config['n_epochs']):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        if use_amp:
            with autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
                ...
                loss = sum([v for k, v in loss_dict.items() if 'accuracy' not in k]) / len([v for k, v in loss_dict.items() if 'accuracy' not in k])  # Only use contrastive_cls loss
            loss.backward()
        else:
            loss_dict = calculate_loss_function(batch)
            loss = sum([v for k, v in loss_dict.items() if 'accuracy' not in k]) / len([v for k, v in loss_dict.items() if 'accuracy' not in k])  # Only use contrastive_cls loss
        ...
        if batch_idx % 1 == 0:
            log(f"Epoch {epoch_i+1}/{training_config['n_epochs']}, Batch {batch_idx+1}/{len(train_dataloader)} ({subject_identifier}_{trial_id}), LR: {optimizers[0].param_groups[0]['lr']:.6f}, Loss: {loss.item():.4f} ({loss_dict['contrastive_cls']:.4f}/{loss_dict['contrastive_x']:.4f}), Temperature: {torch.exp(cross_model.temperature_param).item():.2f}", priority=0)
    ...
    with torch.no_grad():
        ...
        log(f"Test loss: {eval_results['test_loss']:.4f} ({test_loss_dict['contrastive_cls']:.4f}/{test_loss_dict['contrastive_x']:.4f}, Test Accuracy: {test_loss_dict['accuracy_cls']:.4f}/{test_loss_dict['accuracy_x']:.4f}", priority=0)
