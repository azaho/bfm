...
for epoch_i in range(training_config['n_epochs']):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        if use_amp:
            with autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
                ...
                loss = loss_dict['contrastive_x']  # Only use contrastive_x loss
            loss.backward()
        else:
            loss_dict = calculate_loss_function(batch)
            loss = loss_dict['contrastive_x']  # Only use contrastive_x loss
            loss.backward()
        epoch_losses.update(loss_dict)
        ...
        if batch_idx % 1 == 0:
            log(f"Epoch {epoch_i+1}/{training_config['n_epochs']}, Batch {batch_idx+1}/{len(train_dataloader)} ({subject_identifier}_{trial_id}), LR: {optimizers[0].param_groups[0]['lr']:.6f}, Loss: {loss.item():.4f}, Temperature: {torch.exp(cross_model.temperature_param).item():.2f}", priority=0)
    ...
    cross_model.eval()
    electrode_embeddings.eval()
    eval_results = {f"train_{k}": v.item() for k, v in epoch_losses.items()}
    ...
    with torch.no_grad():
        ...
        log(f"Test loss: {test_loss_dict['contrastive_x']:.4f}, Test Accuracy: {test_loss_dict['accuracy_x']:.4f}", priority=0)
