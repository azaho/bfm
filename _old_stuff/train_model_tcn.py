...
bin_transformer = BinTransformer(
    ...
    identity_init=model_config['identity_init']
...
for epoch_i in range(training_config['n_epochs']):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        if batch_idx % 1 == 0:
            log(f"Epoch {epoch_i+1}/{training_config['n_epochs']}, Batch {batch_idx+1}/{len(train_dataloader)} ({subject_identifier}_{trial_id}), LR: {optimizers[0].param_groups[0]['lr']:.6f}, Loss: {loss.item():.4f} ({loss_dict['contrastive_t'].item():.4f}/{loss_dict['contrastive_x'].item():.4f}), Temperature: {torch.exp(model.temperature_param).item():.2f}/{torch.exp(cross_model.temperature_param).item():.2f}", priority=0)
