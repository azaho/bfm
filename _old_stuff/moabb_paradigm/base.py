""" Code originally from: https://github.com/NeuroTechX/moabb/blob/develop/moabb/paradigms/base.py"""
...
from moabb.paradigms import BaseParadigm as MoabbBaseParadigm
from moabb.paradigms import BaseProcessing as MoabbBaseProcessing

from model_electrode_embedding import EEG_CHANNEL_NAME_MAPPING
...
class BaseProcessing(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        self.channel_subset = None #XXX Change from the original code: added this

    # XXX Change from the original code:
    # - Added a possibility to subset the channels to a list of channel names
    def set_channel_subset(self, channel_subset):
        self.channel_subset = [EEG_CHANNEL_NAME_MAPPING.get(x.upper(), x.upper()) for x in channel_subset]
    ...
    def get_data(  # noqa: C901
        ...
        ) + self.make_process_pipelines(dataset, return_epochs, True, postprocess_pipeline) # XXX: Process pipeline twice to ensure that raws are returned. (a bit wasteful but whatever, the moebb codebase is not flexible at all)
                ...
                for subject, sessions in data[0].items():
                    for session, runs in sessions.items():
                        for run, raw in runs.items():
                            raw.pick(self.channel_subset)
            else:
                # Step 1 -- get channel IDs
                for subject, sessions in data[-1].items():
                    for session, runs in sessions.items():
                        for run, raw in runs.items():
                            raw.pick_types(eeg=True)  # First pick only EEG channels
                            original_ch_names = [EEG_CHANNEL_NAME_MAPPING.get(x.upper(), x.upper()) for x in raw.ch_names]
                            ch_ids = []
                            for ch in self.channel_subset:
                                if ch in original_ch_names:
                                    ch_ids.append(original_ch_names.index(ch))
                                else:
                                    print(f"Warning: Channel {ch} not found in the dataset {dataset.code} for subject {subject}, session {session}, run {run}. Setting to copy the first channel {raw.ch_names[0]}.\n\tChannels: {original_ch_names}")
                                    ch_ids.append(0)
                # Step 2 -- subset the data
                for subject, sessions in data[0].items():
                    for session, runs in sessions.items():
                        for run, ordered_dict in runs.items():
                            ordered_dict["X"] = ordered_dict["X"][:, ch_ids, :]
        data = data[:-1] # remove the redundant raw data element
...
class BaseProcessing_old(MoabbBaseProcessing, metaclass=abc.ABCMeta):
    def __init__(
        self,
        filters: List[Tuple[float, float]],
        tmin: float = 0.0,
        ...
        super().__init__(filters, tmin, tmax, baseline, channels, resample)
    ...
    # - Added a possibility to subset the channels to a list of channel names
    def set_channel_subset(self, channel_subset):
        self.channel_subset = [x.upper() for x in channel_subset]
    ...
    def get_data(  # noqa: C901
        ...
        if self.channel_subset is not None:
            data = [
                {subject: 
                    {session: 
                        {run: data[subject][session][run].pick(self.channel_subset) 
...
class BaseParadigm(BaseProcessing, MoabbBaseParadigm):
