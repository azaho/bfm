...
#SBATCH --mem=196G
#SBATCH -t 6:00:00         
#SBATCH --array=1-36 # needs to go to 72
...
random_string_options=("CONT_EXP_EMB1" "CONT_EXP_EMB2" "CONT_EXP_EMB3")
symmetric_loss_options=(1)
learning_rate_options=(0.0015 0.0021 0.003 0.0042 0.006 0.0084)
future_bin_idx_options=(1)
spec_options=(0 1)
embedding_type_options=("learned" "zero" "noisy_coordinate")
...
embedding_type1=${embedding_type_options[$(((idx1 / (n_rs * n_sl * n_lr * n_fb * n_sp)) % n_et))]}
...
embedding_type2=${embedding_type_options[$(((idx2 / (n_rs * n_sl * n_lr * n_fb * n_sp)) % n_et))]}

echo "Job 1 - RS: $random_string1, SL: $symmetric_loss1, LR: $learning_rate1, FB: $future_bin_idx1, SPEC: $spec1, ET: $embedding_type1"
echo "Job 2 - RS: $random_string2, SL: $symmetric_loss2, LR: $learning_rate2, FB: $future_bin_idx2, SPEC: $spec2, ET: $embedding_type2"

# note: change train_model_fbi_combined.py to train_model.py for the non-combined version
python -u train_model_fbi_combined.py  --cache_subjects 1 \
    ...
    --spectrogram $spec1 \
    --electrode_embedding_type $embedding_type1 \
    --n_epochs 100 &
...
python -u train_model_fbi_combined.py  --cache_subjects 1 \
    ...
    --spectrogram $spec2 \
    --electrode_embedding_type $embedding_type2 \
    --n_epochs 100 &
