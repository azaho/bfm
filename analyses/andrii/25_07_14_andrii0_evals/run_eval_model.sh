...
CLASSIFIER_TYPE_IDX=$(( ($SLURM_ARRAY_TASK_ID-1) / ${#eval_names[@]} / ${#subjects[@]} / ${#splits_type[@]} % ${#classifier_type[@]} ))
MODEL_DIR_IDX=$(( ($SLURM_ARRAY_TASK_ID-1) / ${#eval_names[@]} / ${#subjects[@]} / ${#splits_type[@]} / ${#classifier_type[@]} % ${#model_dirs[@]} ))
MODEL_EPOCH_IDX=$(( ($SLURM_ARRAY_TASK_ID-1) / ${#eval_names[@]} / ${#subjects[@]} / ${#splits_type[@]} / ${#classifier_type[@]} / ${#model_dirs[@]} % ${#model_epochs[@]} ))
FEATURE_TYPE_IDX=$(( ($SLURM_ARRAY_TASK_ID-1) / ${#eval_names[@]} / ${#subjects[@]} / ${#splits_type[@]} / ${#classifier_type[@]} / ${#model_dirs[@]} / ${#model_epochs[@]} % ${#feature_type[@]} ))
...
save_dir="runs/analyses/andrii/25_07_14_andrii0_evals/eval_results_lite_${SPLITS_TYPE}_${FEATURE_TYPE}"

echo "Running eval for eval $EVAL_NAME, subject $SUBJECT, trial $TRIAL, classifier $CLASSIFIER_TYPE, model $MODEL_DIR, epoch $MODEL_EPOCH, feature type $FEATURE_TYPE"
