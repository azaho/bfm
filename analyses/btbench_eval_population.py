...
for eval_name in eval_names:
    save_file_path = os.path.join(save_dir, model_dir, f"frozen_population_btbank{subject_id}_{trial_id}_{eval_name}.json")
    ...
    results_population["onset_1sec_window"] = {
        "time_bin_start": float(-bins_start_before_word_onset_seconds),
        "time_bin_end": float(bins_end_after_word_onset_seconds),
        "folds": []
    }
    ...
    for fold_idx in range(len(train_datasets)):
        ...
        for bin_idx in range(-2, 0):# len(bin_starts)):
            ...
            if bin_idx == -2: # just take a 1 second window centered on the word onset
                if timebins_aggregation == "concat":
                    X_train = X_train_all_bins[:, 4:12, :].reshape(X_train_all_bins.shape[0], -1).float().cpu().numpy()
                    X_test = X_test_all_bins[:, 4:12, :].reshape(X_test_all_bins.shape[0], -1).float().cpu().numpy()
                else:
                    X_train = X_train_all_bins[:, 4:12, :].mean(dim=1).float().cpu().numpy()
                    X_test = X_test_all_bins[:, 4:12, :].mean(dim=1).float().cpu().numpy()
            elif bin_idx == -1:
                if timebins_aggregation == "concat":
            ...
            if bin_idx == -2:
                results_population["onset_1sec_window"]["folds"].append(fold_result)
            elif bin_idx == -1:
                results_population["whole_window"]["folds"].append(fold_result)
            else:
                results_population["time_bins"][bin_idx]["folds"].append(fold_result)
            if verbose: print(f"Subject {subject.subject_identifier} trial {trial_id}, Fold {fold_idx+1}, Bin {bin_start}-{bin_end}: Train accuracy: {train_accuracy:.3f}, Test accuracy: {test_accuracy:.3f}, Train ROC AUC: {train_roc:.3f}, Test ROC AUC: {test_roc:.3f}")
