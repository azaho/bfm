...
def calculate_loss_function(batch, output_accuracy=True):
    ...
    bin_embed_transformed_data = bin_embed_transformer(batch['data']) # shape: (batch_size, n_electrodes, n_timebins, sample_timebin_size*SR//n_downsample_factor)
    if model_config['separate_unembed']:
        bin_unembed_transformed_data = bin_unembed_transformer(batch['data']) # shape: (batch_size, n_electrodes, n_timebins, sample_timebin_size*SR//n_downsample_factor)
    else:
        bin_unembed_transformed_data = bin_embed_transformed_data.clone()

    batch_size, n_electrodes, n_timebins, sample_timebin_size = bin_embed_transformed_data.shape
    bin_embed_transformed_data_copy = bin_embed_transformed_data[:, :, :, :].clone()

    
    embeddings = electrode_embeddings.forward(batch['electrode_index']).unsqueeze(-2).repeat(1, 1, n_timebins-future_bin_idx, 1) # shape: (batch_size, n_electrodes, d_model, n_timebins-future_bin_idx)
    embeddings_copy = embeddings.clone()
    
    # Zero out appropriate % of embeddings
    zero_embed_indices = np.random.choice(electrodes_a, size=int(len(electrodes_a)*(1-training_config['p_show_a_embedding'])), replace=False)
    embeddings[:, zero_embed_indices, :] = 0
    zero_embed_indices = np.random.choice(electrodes_b, size=int(len(electrodes_b)*(1-training_config['p_show_b_embedding'])), replace=False)
    embeddings[:, zero_embed_indices, :] = 0

    # This will be used just for the CLS token.
    zero_embed_indices = np.random.choice(electrodes_b, size=int(len(electrodes_b)*(1-training_config['p_show_a_embedding'])), replace=False)
    embeddings_copy[:, zero_embed_indices, :] = 0


    masked_tokens = torch.zeros(batch_size, n_electrodes, n_timebins, device=model.device, dtype=model.dtype)
    bin_embed_transformed_data[:, electrodes_b, :, :] = 0 # mask out the second half of the electrodes
    
    zero_time_indices = np.random.choice(n_timebins, size=int(n_timebins*(1-training_config['p_masked_timebins'])), replace=False)
    bin_embed_transformed_data[:, :, zero_time_indices, :] = 0


    masked_tokens[:, electrodes_b, :] = 0 # removed the mask tokens again 


    output, output_a_cls = model(bin_embed_transformed_data[:, :, :-future_bin_idx, :], embeddings, masked_tokens=masked_tokens[:, :, :-future_bin_idx], return_cls_token=True) # shape: (batch_size, n_electrodes, n_timebins-future_bin_idx, sample_timebin_size*SR//n_downsample_factor)
    target = bin_unembed_transformed_data[:, :, future_bin_idx:, :] # shape: (batch_size, n_electrodes, n_timebins-future_bin_idx, sample_timebin_size*SR//n_downsample_factor)
    ...
    _, output_b_cls = model(bin_embed_transformed_data_copy[:, electrodes_b, future_bin_idx:, :], embeddings_copy[:, electrodes_b, :], return_cls_token=True, strict_positions=True)
