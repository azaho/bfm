...
# 2. (batch_size, n_electrodes, n_timebins, max_frequency_bin) -> single transformer -> (batch_size, n_electrodes * n_timebins + 1, d_model)
# 3. Extract CLS token -> (batch_size, d_model)
# loss function: compare the CLS tokens from two subjects using contrastive loss
###
...
class SimpleMSEAutoregressiveModel(BFModule):
    def __init__(self, d_model, spectrogram_parameters, d_input, n_layers=5, n_heads=12, dropout=0.1):
        ...
        self.transformer = Transformer(d_input=d_input, d_model=d_model, 
                                        d_output=d_model,  # Changed to d_model for CLS token output
                                        n_layer=n_layers, n_head=n_heads, causal=False,  # Changed to non-causal for contrastive learning
                                        rope=False, dropout=dropout)
        
        # CLS token for the shared latent space
        self.cls_token = nn.Parameter(torch.zeros(1, d_input))  # Use d_input to match transformer input
    ...
    def forward(self, electrode_data, embeddings=None, only_cls_token=False):
        ...
        # Add electrode embeddings if provided
        if embeddings is not None:
            electrode_data = electrode_data + embeddings.unsqueeze(2) # shape: (batch_size, n_electrodes, n_timebins, d_input)
        
        # Reshape to combine all electrodes and time tokens
        ...
        electrode_data = torch.cat([cls_tokens, electrode_data], dim=1) # shape: (batch_size, n_electrodes * n_timebins + 1, d_input)
        
        # Pass through transformer
        transformed_data = self.transformer(electrode_data) # shape: (batch_size, n_electrodes * n_timebins + 1, d_model)
        
        if only_cls_token:
            # Return only the CLS token
            return transformed_data[:, 0, :] # shape: (batch_size, d_model)
        else:
            # Return all tokens
            return transformed_data # shape: (batch_size, n_electrodes * n_timebins + 1, d_model)

### DEFINING THE TRAINING SETUP ###

class mse_ar_podcast(TrainingSetup):
    ...
    def initialize_model(self):
        ...
        self.fft_preprocessor = SpectrogramPreprocessor(output_dim=config['model']['transformer']['d_model'], max_frequency=200)
    ...
    def calculate_pretrain_loss(self, batch, output_accuracy=True):
        ...
        def _add_to_loss_contrastive(losses, output, target, loss_suffix):
            # output and target shape: (batch_size, d_model) - CLS tokens from two subjects
            ...
            # Compute similarity matrix between CLS tokens
            similarity = output_ @ target_.T # shape: (batch_size, batch_size)
            ...
            labels = torch.arange(batch_size, device=self.model.device, dtype=torch.long)
            ...
            if output_accuracy:
                ...
                losses[f'accuracy_{loss_suffix}'] = accuracy
        ...
        # Preprocess data through STFT
        preprocessed_data_a = self.fft_preprocessor(batch_a) # shape: (batch_size, n_electrodes, n_timebins, d_model)
        preprocessed_data_b = self.fft_preprocessor(batch_b) # shape: (batch_size, n_electrodes, n_timebins, d_model)
        
        # Pass through single transformer and extract CLS tokens
        cls_token_a = self.model(preprocessed_data_a, embeddings_a, only_cls_token=True) # shape: (batch_size, d_model)
        cls_token_b = self.model(preprocessed_data_b, embeddings_b, only_cls_token=True) # shape: (batch_size, d_model)

        # Contrastive loss: compare CLS tokens between subjects
        # Subject A's CLS token predicts Subject B's CLS token
        losses = _add_to_loss_contrastive(losses, cls_token_a, cls_token_b, 'a')
        # Subject B's CLS token predicts Subject A's CLS token  
        losses = _add_to_loss_contrastive(losses, cls_token_b, cls_token_a, 'b')
    ...
    def generate_frozen_features(self, batch):
        ...
        # OUTPUT:
        #   features shape: (batch_size, d_model) - just the CLS token
        batch['data'] = batch['data'].to(self.model.device, dtype=self.model.dtype, non_blocking=True)
        batch['electrode_index'] = batch['electrode_index'].to(self.model.device, non_blocking=True)

        embeddings = self.electrode_embeddings(batch)
        preprocessed_data = self.fft_preprocessor(batch) # shape: (batch_size, n_electrodes, n_timebins, d_model)
        features = self.model(preprocessed_data, embeddings, only_cls_token=True) # shape: (batch_size, d_model)
    ...
    def load_dataloaders(self):
        ...
        Uses temporal splitting to avoid temporal contamination.
        """
        config = self.config
        ...
        for subject_a_id, subject_b_id in train_subject_pairs:
            ...
            # SEQUENTIAL TEMPORAL SPLITTING (first 80% train, last 20% test)
            # Use sequential split to avoid temporal leakage
            ...
            if self.verbose:
                log(f"Finished creating paired dataset: {len(dataset)} windows (train: {len(train_dataset)}, test: {len(test_dataset)})", indent=1, priority=1)
