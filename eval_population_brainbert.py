...
parser.add_argument('--time_granularity', type=int, default=1, help='Time granularity of the model features (how many bins within a window)')
parser.add_argument('--model_path', type=str, default="eval_results/juno4/btbank9_0_embed192_nspb4_model.pt", help='Path to the model checkpoint')
...
model_config = checkpoint['training_config']
d_embed = model_config['d_embed']

n_samples_per_bin = model_config['n_samples_per_bin']
window_size = model_config['window_size']

# Initialize model components
embed = EmbedderLinear(d_model=d_embed, d_input=n_samples_per_bin).to(device)
unembed = EmbedderLinear(d_model=d_embed, d_input=n_samples_per_bin).to(device)
inverter = DistributionInverter(samples=torch.tensor([0.0, 1.0])).to(device)
...
log("Model loaded successfully", priority=0)

def get_model_features(model, x, max_batch_size=32):
    """
    x: np.ndarray of shape (batch_size, n_channels, n_samples)
    returns: np.ndarray of shape (batch_size, n_channels, n_timebins, d_embed)
    """
    ...
    with torch.no_grad():
        # Move model to GPU once at the start
        ...
        for i in range(0, batch_size, max_batch_size):
            ...
            batch_i, n_channels, n_samples = batch_x.shape
            
            # Reshape to match model input requirements
            log("Converting batch to tensor", priority=4, indent=3)
            batch_x_tensor = torch.tensor(batch_x, dtype=dtype, device=device)
            
            log("Reshaping tensor for model input", priority=4, indent=3)
            batch_x_tensor = batch_x_tensor.reshape(batch_i * n_channels, n_samples//n_samples_per_bin, n_samples_per_bin)
            
            log("Applying inverter to batch", priority=4, indent=3)
            batch_out = inverter(batch_x_tensor) # shape (batch_i * n_channels, n_samples//n_samples_per_bin, n_samples_per_bin)
            
            log("Adding dimension to batch output", priority=4, indent=3)
            batch_out = batch_out.unsqueeze(-2) # shape (batch_i * n_channels, n_samples//n_samples_per_bin, 1, n_samples_per_bin)
            
            log("Reshaping batch for model processing", priority=4, indent=3)
            batch_out = batch_out.reshape(batch_i, n_channels, batch_out.shape[1], batch_out.shape[3])

            # Stack channels
            log("Running model forward pass", priority=4, indent=3)
            batch_out = model(batch_out)
            
            # Time-bin pooling if needed
            if time_granularity != 1:
                log("Performing time-bin pooling", priority=4, indent=3)
                seq_len = batch_out.shape[2]
                timebin_size = seq_len // time_granularity
                n_timebins = seq_len // timebin_size
                batch_out = batch_out[:, :, :(n_timebins * timebin_size), :]
                batch_out = batch_out.reshape(batch_i, n_channels, n_timebins, timebin_size, d_embed)
                batch_out = batch_out.mean(dim=3)  # (batch_i, n_channels, n_timebins, d_embed)
            
            log("Converting output to numpy", priority=4, indent=3)
            ...
            if out_array is None:
                log("Allocating output array", priority=4, indent=3)
                out_array = np.empty((batch_size, n_channels, batch_np.shape[2], d_embed), dtype=batch_np.dtype)

            # fill slice
            log("Filling output array slice", priority=4, indent=3)
...
subject = BTBench_BrainTreebankSubject(subject_id, allow_corrupted=False, cache=True, dtype=torch.float32)
all_electrode_labels = subject.electrode_labels

for eval_name in eval_names:
    file_save_dir = f"{save_dir}/transformer_frozen_mean_granularity_{time_granularity}"
    ...
    for bin_start, bin_end in zip(bin_starts, bin_ends):
        ...
        for fold_idx in range(len(train_datasets)):
            ...
            X_train = get_model_features(model, X_train)
            X_test = get_model_features(model, X_test)
    ...
    results = {
        "model_name": "Transformer (frozen) _ logistic regression",
        "author": "Andrii Zahorodnii",
        "description": f"Transformer features (frozen) + logistic regression, all electrodes' features concatenated, mean across all timesteps within a timebin (splitting the window into {time_granularity} timebins).",
        "organization": "MIT",
