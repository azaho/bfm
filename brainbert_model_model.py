...
class ElectrodeTransformer(BFModule):
    ...
    def generate_frozen_evaluation_features(self, electrode_data, embeddings, feature_aggregation_method='concat'):
        # electrode_data is of shape (batch_size, n_electrodes, n_timebins, d_model)
        # embeddings is of shape (batch_size, n_electrodes, d_model)
        features = self.forward(electrode_data, embeddings) # shape: (batch_size, n_timebins, d_model)
...
class OriginalModel(BFModule):
    def __init__(self, d_model, n_layers=5, n_heads=12, dropout=0.1):
        ...
        self.electrode_transformer = ElectrodeTransformer(d_model, n_layers, n_heads, dropout)
        self.time_transformer = TimeTransformer(d_model, n_layers, n_heads, dropout)
    ...
    def forward(self, electrode_data, embeddings=None):
        ...
        return time_transformed_data
    
    def generate_frozen_evaluation_features(self, electrode_data, embeddings, feature_aggregation_method='concat'):
        return self.electrode_transformer.generate_frozen_evaluation_features(electrode_data, embeddings, feature_aggregation_method)
