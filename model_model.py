...
class GranularModel(BFModel):
    ...
    def forward(self, embedded_electrode_data):
        # embedded_electrode_data is of shape (batch_size, n_electrodes, n_timebins, d_model)
        batch_size, n_electrodes, n_timebins, d_model = embedded_electrode_data.shape
        electrode_data = embedded_electrode_data.transpose(1, 2) # shape: (batch_size, n_timebins, n_electrodes, d_model)
        electrode_data = embedded_electrode_data.reshape(batch_size, n_electrodes * n_timebins, d_model)
        ...
        electrode_output = self.transformer(electrode_data, positions=positions) # shape: (batch_size, n_electrodes * n_timebins, d_model)
        electrode_output = electrode_output.reshape(batch_size, n_timebins, n_electrodes, d_model).transpose(1, 2) # shape: (batch_size, n_electrodes, n_timebins, d_model)
    ...
    def generate_frozen_evaluation_features(self, electrode_embedded_data, feature_aggregation_method='concat'):
        ...
        output = self(electrode_embedded_data) # shape: (batch_size, n_electrodes, n_timebins, d_model)
