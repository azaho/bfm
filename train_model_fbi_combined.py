...
model = TransformerModel(
    ...
    d_output=int(model_config['transformer']['d_model'] * training_config['future_bin_idx']),
...
def calculate_loss_function(batch, subject_identifier, trial_id):
    ...
    o1_e, o1_t = model(electrode_embedded_data[:, :n_electrodes_per_stream, :, :]) # shape: (batch_size, n_timebins, d_model) and (batch_size, n_timebins, d_model * future_bin_idx)
    o2_e, o2_t = model(electrode_embedded_data[:, -n_electrodes_per_stream:, :, :]) # shape: (batch_size, n_timebins, d_model) and (batch_size, n_timebins, d_model * future_bin_idx)
    
    o1_t = o1_t.reshape(batch_size, n_timebins, training_config['future_bin_idx'], d_model)
    o2_t = o2_t.reshape(batch_size, n_timebins, training_config['future_bin_idx'], d_model)

    loss = 0
    loss_dict = {}

    for fbi in range(training_config['future_bin_idx']):
        fbi_loss = 0

        similarity_1 = torch.matmul(o1_t[:, :-training_config['future_bin_idx'], fbi].permute(1, 0, 2), o2_e[:, training_config['future_bin_idx']:].permute(1, 2, 0)) * model.temperature_param
        ...
        fbi_loss += torch.nn.functional.cross_entropy(similarity_1.view(-1, batch_size), expanded_arange)

        if training_config['symmetric_loss']:
            similarity_2 = torch.matmul(o2_t[:, :-training_config['future_bin_idx'], fbi].permute(1, 0, 2), o1_e[:, training_config['future_bin_idx']:].permute(1, 2, 0)) * model.temperature_param
            fbi_loss += torch.nn.functional.cross_entropy(similarity_2.view(-1, batch_size), expanded_arange)
            fbi_loss /= 2

        loss += fbi_loss
        loss_dict[f"contrastive_fbi{fbi}"] = fbi_loss
