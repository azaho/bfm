...
# Helper script to submit movie processing job with correct array size
# Usage: sbatch stimulus_feature_extraction_pipeline_brainbert.sh
...
movie_count=$(find "$MOVIES_DIR" -type f \( -iname "*.mp4" -o -iname "*.avi" -o -iname "*.mov" -o -iname "*.mkv" -o -iname "*.wmv" -o -iname "*.flv" -o -iname "*.webm" -o -iname "*.m4v" \) | wc -l)

echo "Found $movie_count movie files"

if [ $movie_count -eq 0 ]; then
    echo "No movie files found in $MOVIES_DIR"
    exit 1
fi

# Create logs directory if it doesn't exist
mkdir -p /bfm/runs/logs

# Create a temporary script with the correct array size
temp_script=$(mktemp)
cat > "$temp_script" << EOF
#!/bin/bash
...
#SBATCH --mem=64G
#SBATCH -t 24:00:00
#SBATCH --output /bfm/runs/logs/movie_preprocessing_%A_%a.out
#SBATCH --error /bfm/runs/logs/movie_preprocessing_%A_%a.err
#SBATCH --array=1-$movie_count
#SBATCH -p normal
...
# Set default values for environment variables
export MOVIES_DIR=\${MOVIES_DIR:-"/om2/data/public/braintreebank_movies"}
export OUTPUT_BASE_DIR=\${OUTPUT_BASE_DIR:-"/om2/data/public/braintreebank_movies_preprocessed"}
...
# CLIP features
process_pipeline "clip" "\$movie_path" "\$movie_name" "\$FRAME_INTERVAL" "\$OUTPUT_BASE_DIR/clip_features" "/bfm/model/preprocessing/stimulus_feature_extraction/clip_preprocessing.py" "" &

# DINOv2 features  
process_pipeline "dinov2" "\$movie_path" "\$movie_name" "\$FRAME_INTERVAL" "\$OUTPUT_BASE_DIR/dinov2_features" "/bfm/model/preprocessing/stimulus_feature_extraction/dinov2_preprocessing.py" "" &

# Audio features
process_pipeline "audio" "\$movie_path" "\$movie_name" "\$AUDIO_FRAME_INTERVAL" "\$OUTPUT_BASE_DIR/audio_features" "/bfm/model/preprocessing/stimulus_feature_extraction/audio_preprocessing.py" "--segment_length \$AUDIO_SEGMENT_LENGTH --model_name \$AUDIO_MODEL" &
...
echo "Submitting SLURM job for $movie_count movies..."
echo "Each movie will have all three pipelines (CLIP, DINOv2, Audio) processed in parallel"
echo ""
...
echo "  squeue -u $USER"
echo "  tail -f runs/logs/movie_preprocessing_${job_id}_*.out"
echo ""
